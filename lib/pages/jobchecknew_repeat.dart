...
class JobCheckNewRepeatPage extends StatefulWidget {
  final String plan_id;
  final String plan_area_id;
  final String plan_area_name;
  final String plan_num;
  const JobCheckNewRepeatPage({
    Key? key,
  ...
  @override
  State<JobCheckNewRepeatPage> createState() => _JobCheckNewRepeatPageState();
}

class _JobCheckNewRepeatPageState extends State<JobCheckNewRepeatPage> {
  ...
  dynamic _addselected(String score, dynamic topic_id, dynamic topic_item_id) {
    if (score != '') {
      ...
      Provider.of<PlanData>(context, listen: false)
          .addInspectionTransRepeat(_inspec);
              ...
              Expanded(
                child: Consumer<PlanData>(
                    builder: (context, topicitems, _) => _buildlist(
                        topicitems.getTopicitemNewRepeat(widget.plan_area_id))),
              ),
            ],
          )),
    );
